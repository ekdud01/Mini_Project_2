...
| v2.8 | 2026-09-26 | 채점 책임을 Entity 설계서와 일치(점수 산출은 결과 도메인 서비스 ScoringService, 판정은 SurveyResult), UML 메서드 정리, 비밀번호 규칙(BR-011) 추가 |
...
**주요 행동 (메서드):**
- 점수 산출: 제출된 답변을 문항 번호로 바꿔 1차 점수와 영역별 점수를 합산한다. 문항 조회가 필요하므로 결과 도메인 서비스(`ScoringService`)가 담당하고, 산출된 점수로 SurveyResult를 생성한다
- `getTotalScore()`: KDSQ-C 총점(영역별 점수 합) 계산. KDSQ-P로 끝난 결과는 null
...
classDiagram
    ...
    ScoringService ..> SurveyResult : 점수 산출 후 생성
    ScoringService ..> Question : 문항 번호 매핑
```

> Solution은 SurveyResult와 FK로 연결하지 않고, 결과의 `riskLevel`로 조회하는 의존 관계(점선)이다. ScoringService는 저장되는 객체가 아닌 결과 도메인 서비스로, 답변을 채점해 SurveyResult를 만든다. 위험도 판정(`evaluateRisk()`)은 SurveyResult 내부에서 생성 시 수행한다.
...
| BR-011 | 비밀번호는 8~20자, 영문·숫자·특수문자 각 1개 이상 포함해야 하며, BCrypt로 암호화하여 저장 | Member |
